...
namespace Terrain3DTools.Core
{
    ...
    public static class LayerDependencyManager
    {
        ...
        /// <summary>
        /// Propagates dirty state to instancer layers that would be affected by
        /// another instancer's region clearing. This ensures instances aren't
        /// lost when multiple instancers share mesh IDs and regions.
        /// </summary>
        public static void PropagateInstancerDirtyState(
            IEnumerable<TerrainLayerBase> allLayers,
            HashSet<TerrainLayerBase> dirtyLayers,
            System.Collections.Generic.Dictionary<ulong, HashSet<Vector2I>> previousInstanceRegions,
            int regionSize)
        {
            // Get dirty instancer layers
            var dirtyInstancers = dirtyLayers
                .OfType<InstancerLayer>()
                .Where(l => GodotObject.IsInstanceValid(l))
                .ToList();

            if (dirtyInstancers.Count == 0) return;

            // Get all instancer layers (including non-dirty ones)
            var allInstancers = allLayers
                .OfType<InstancerLayer>()
                .Where(l => GodotObject.IsInstanceValid(l))
                .ToList();

            // For each dirty instancer, find other instancers that need to regenerate
            foreach (var dirtyInstancer in dirtyInstancers)
            {
                var dirtyLayerId = dirtyInstancer.GetInstanceId();
                var dirtyMeshIds = dirtyInstancer.GetMeshAssetIds().ToHashSet();

                if (dirtyMeshIds.Count == 0) continue;

                // Get regions that will be cleared (current + previous)
                var regionsToBeClearedForDirty = new HashSet<Vector2I>();

                // Current regions
                var currentBounds = TerrainCoordinateHelper.GetRegionBoundsForLayer(dirtyInstancer, regionSize);
                foreach (var coord in currentBounds.GetRegionCoords())
                {
                    regionsToBeClearedForDirty.Add(coord);
                }

                // Previous regions
                if (previousInstanceRegions.TryGetValue(dirtyLayerId, out var prevRegions))
                {
                    foreach (var coord in prevRegions)
                    {
                        regionsToBeClearedForDirty.Add(coord);
                    }
                }

                // Check other instancers for overlap
                foreach (var otherInstancer in allInstancers)
                {
                    if (otherInstancer == dirtyInstancer) continue;
                    if (dirtyLayers.Contains(otherInstancer)) continue; // Already dirty

                    var otherMeshIds = otherInstancer.GetMeshAssetIds().ToHashSet();

                    // Check if they share any mesh IDs
                    if (!dirtyMeshIds.Overlaps(otherMeshIds)) continue;

                    // Check if they overlap any regions being cleared
                    var otherBounds = TerrainCoordinateHelper.GetRegionBoundsForLayer(otherInstancer, regionSize);
                    var otherRegions = otherBounds.GetRegionCoords().ToHashSet();

                    if (otherRegions.Overlaps(regionsToBeClearedForDirty))
                    {
                        // This instancer will lose instances when we clear - mark it dirty
                        otherInstancer.ForceDirty();
                        dirtyLayers.Add(otherInstancer);

                        DebugManager.Instance?.Log("LayerDependencyManager", DebugCategory.LayerDirtying,
                            $"Propagated dirty to '{otherInstancer.LayerName}' - shares mesh IDs and regions with '{dirtyInstancer.LayerName}'");
                    }
                }
            }
        }
