...
namespace Terrain3DTools.Core
{
    ...
    public static class GpuKernels
    {
        ...
        /// <summary>
        /// Creates GPU compute commands to copy one texture to another.
        /// Uses a compute shader for proper synchronization within compute pipelines.
        /// </summary>
        /// <param name="sourceTexture">Source texture (must have SamplingBit)</param>
        /// <param name="destinationTexture">Destination texture (must have StorageBit)</param>
        /// <param name="width">Texture width</param>
        /// <param name="height">Texture height</param>
        /// <param name="debugClassName">Calling class for debug logging</param>
        /// <returns>Tuple of (GPU commands, temporary RIDs, shader path)</returns>
        public static (Action<long> commands, List<Rid> tempRids, string shaderPath) CreateCopyTextureCommands(
            Rid sourceTexture,
            Rid destinationTexture,
            int width,
            int height,
            string debugClassName = null)
        {
            var shaderPath = "res://addons/terrain_3d_tools/Shaders/Utils/copy_texture.glsl";
            ...
            if (!sourceTexture.IsValid || !destinationTexture.IsValid)
            {
                ...
                {
                    DebugManager.Instance?.LogError(debugClassName,
                        $"Invalid RIDs for copy: source={sourceTexture.IsValid}, dest={destinationTexture.IsValid}");
                }
                return (null, new List<Rid>(), shaderPath);
            }
            ...
            operation.BindSamplerWithTexture(0, sourceTexture);
            operation.BindStorageImage(1, destinationTexture);
            operation.SetPushConstants(new byte[0]);
