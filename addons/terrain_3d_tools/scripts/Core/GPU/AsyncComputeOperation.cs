...
namespace Terrain3DTools.Core
{
    ...
    public class AsyncComputeOperation
    {
        ...
        public Rid Shader => _shader;
        public Rid Pipeline => _pipeline;
        #endregion
        ...
        public void BindUniformBuffer(uint binding, Rid bufferRid)
        {
            if (_hasErrors) return;
            ...
            {
                DebugManager.Instance?.LogError(DEBUG_CLASS_NAME,
                    $"Cannot bind invalid uniform buffer RID at binding {binding} for shader '{_shaderPath}'");
                _hasErrors = true;
                return;
            }

            _uniforms.Add(GpuUtils.CreateUniformBufferUniform(binding, bufferRid));

            DebugManager.Instance?.Log(DEBUG_CLASS_NAME, DebugCategory.ShaderOperations,
                $"Bound uniform buffer at binding {binding}");
        }
