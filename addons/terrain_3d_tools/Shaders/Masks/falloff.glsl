...
float sampleCurve(float t) {
    ...
    // Get the two integer indices surrounding the fractional one
    int idx0 = int(f_idx);
    int idx1 = min(idx0 + 1, curve.pointCount - 1); // Clamp to avoid going out of bounds
    
    // Get the values at those two points
    ...
    // Linearly interpolate (lerp) between the two values
