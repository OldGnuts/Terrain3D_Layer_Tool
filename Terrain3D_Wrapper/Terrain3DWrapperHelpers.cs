...
namespace Terrain3DWrapper
{
    ...
    public static class Terrain3DNodeVerifier
    {
        public static bool CheckTerrain3D(Node node)
        {
            if (ClassDB.ClassExists("Terrain3D"))
            {
                if (node.IsClass("Terrain3D"))
                {
                    GD.Print("valid Terrain3D node supplied");
                    return true;
                }
                else
                {
                    GD.PrintErr("Invalid node supplied or not a Terrain3D node");
                    return false;
                }
            }
            GD.PrintErr("Terrain3D class not found");
            return false;
        }
    }
    /// <summary>
