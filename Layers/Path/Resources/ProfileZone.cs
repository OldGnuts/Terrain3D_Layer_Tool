...
namespace Terrain3DTools.Layers.Path
{
    ...
    public partial class ProfileZone : Resource
    {
        ...
        #region GPU Data
        /// <summary>
        /// Pack zone data for GPU with default scale (world units).
        /// </summary>
        public float[] ToGpuData()
        {
            return ToGpuData(1.0f);
        }

        /// <summary>
        /// Pack zone data for GPU with optional scaling for width.
        /// </summary>
        /// <param name="widthScale">Scale factor for width (1.0 = world units, >1 = pixel space)</param>
        public float[] ToGpuData(float widthScale)
        {
            var data = new System.Collections.Generic.List<float>
            {
                // Zone basic info (16 bytes, 4 floats)
                _enabled ? 1.0f : 0.0f,
                _width * widthScale,  // Width is scaled
                _heightOffset,        // Height offset stays in world units
                _heightStrength,
